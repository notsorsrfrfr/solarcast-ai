...
<style>
    ...
    [data-testid="stSidebar"] { background-color: #0d1425; }
...
<div style="margin-bottom: 1.5rem;">
    <div>
        <span class="sc-header">SolarCast AI</span>
...
with c4:
    anomaly_count = anomaly_data['total_anomalies'] if anomaly_data else 0
    delta_class = "warn" if anomaly_count > 0 else ""
    st.markdown(f"""
    <div class="kpi-card">
        <div class="kpi-label">Anomalies Detected</div>
        <div class="kpi-value">{anomaly_count}<span class="kpi-unit">events</span></div>
        <div class="kpi-delta {delta_class}">{'Requires attention' if anomaly_count > 0 else 'All systems normal'}</div>
    </div>""", unsafe_allow_html=True)
    ...
    if forecast_data:
        ...
        fig.update_layout(
            ...
            yaxis=dict(
                ...
                linecolor='#1a2a4a',
                title='MW',
                title_font=dict(color='#3a5a8a', size=11)
            ),
            legend=dict(
        ...
        m3.metric("Avg Next 24h", f"{round(df.head(24)['yhat'].mean(), 1)} MW")

with col_right:
    st.markdown('<div class="section-title">Anomaly Alerts</div>', unsafe_allow_html=True)

    if anomaly_data and anomaly_data['total_anomalies'] > 0:
        for a in anomaly_data['anomalies'][:7]:
            ts = str(a['timestamp'])
...
if forecast_data:
    df24 = pd.DataFrame(forecast_data['forecast']).head(24)
    df24['ds'] = pd.to_datetime(df24['ds'])
    df24['hour'] = df24['ds'].dt.strftime('%H:%M')
    df24['color'] = df24['yhat'].apply(
        lambda x: '#00c896' if x > 500 else ('#f0a040' if x > 150 else '#c0392b')
    )
